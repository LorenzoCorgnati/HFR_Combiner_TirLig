...
%     end
    ...
    for rss=start_time:stop_time % Sweeps RadStruct
        if (RadStruct(rss).processedFlag == 0) % Checks if the group of radials has to be processed
            ...
            %             if (num_input > 1)
            RadStruct(rss).corruptedFlag = SingleTotal_v40(inputfiles, RadStruct(rss).TS, lon_lim, lat_lim, Grid, spatthresh, Radial_QC_params, Total_QC_params, range_cells_number, ncst, total_maps_folder, tuv_local_folder, tuv_serv_folder, src_folder, total_maps_server_folder, netcdf_thredds_folder);
