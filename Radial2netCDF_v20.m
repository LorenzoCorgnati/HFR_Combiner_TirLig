...
function [R2C_err] = Radial2netCDF_v20(HFRP_RUV, range_cells_number, Radial_QC_params, dest_nc_local, dest_nc_tds, dest_nc_rd)
...
%    velv(rb_map_idx) = 0;
    ...
    %% Perform QC tests
    [overall_QCflag, overWater_QCflag, varianceThreshold_QCflag, velocityThreshold_QCflag, medianFilter_QCflag, averageRadialBearing_QC_flag, radialVelocityMedianFiltered] = RadialQCtests_v10(bear, lond, latd, owtr, etmp, head, velo, Radial_QC_params);
    ...
    dimid_range = netcdf.defDim(ncid, 'RANG', numel( range_dim));
    dimid_depth = netcdf.defDim(ncid, 'DEPH', 1);
    dimid_nsit = netcdf.defDim(ncid, 'NSIT', 1);
    dimid_smxl = netcdf.defDim(ncid, 'SMXL', 9);
    dimid_qmxl = netcdf.defDim(ncid, 'QMXL', 1);
    ...
    netcdf.putAtt(ncid, varid_t, 'units', ['seconds since ' time_units]);
...
%     netcdf.putAtt(ncid, varid_t, 'FillValue', netcdf.getConstant('NC_FILL_FLOAT'));
    ...
    varid_range = netcdf.defVar(ncid, 'RANG', 'float', dimid_range);
...
%     netcdf.putAtt(ncid, varid_depth, 'FillValue', netcdf.getConstant('NC_FILL_FLOAT'));
    ...
    % (CF, ACDD, OceanSITES) source and network
    netcdf.putAtt(ncid, varid_global, 'source', 'coastal structure');
    netcdf.putAtt(ncid, varid_global, 'network', 'HF-Radar Ocean Surface Observation');
    ...
    % (ACDD, OceanSITES) format version
    netcdf.putAtt(ncid, varid_global, 'format_version', 'v1.0');
    
    % (OceanSITES) format version
    netcdf.putAtt(ncid, varid_global, 'data_mode', 'R');
    
    % (OceanSITES) update interval
    ...
    % (OceanSITES) site code and coverage area
    netcdf.putAtt(ncid, varid_global, 'site_code', '');
    ...
    netcdf.putAtt( ncid, varid_lat, 'coordinates', 'TIME DEPH HEAD RANG' );
...
%     netcdf.putAtt(ncid, varid_lat, 'axis', 'Y');
    ...
    netcdf.putAtt( ncid, varid_lon, 'coordinates', 'TIME DEPH HEAD RANG' );
...
%     netcdf.putAtt(ncid, varid_v, 'cell_methods', 'time: mean over hours time');
    ...
    netcdf.putAtt(ncid, varid_ovqc, 'scale_factor', single(1));
    netcdf.putAtt(ncid, varid_ovqc, 'add_offset', single(0));
    ...
    netcdf.putAtt(ncid, varid_owtr, 'scale_factor', single(1));
    netcdf.putAtt(ncid, varid_owtr, 'add_offset', single(0));
    ...
    netcdf.putAtt(ncid, varid_mdfl, 'comment', 'OceanSITES quality flagging for Median Filter QC test. Threshold set to m/s.');
    netcdf.putAtt(ncid, varid_mdfl, 'FillValue', netcdf.getConstant('NC_FILL_SHORT'));
    netcdf.putAtt(ncid, varid_mdfl, 'scale_factor', single(1));
    netcdf.putAtt(ncid, varid_mdfl, 'add_offset', single(0));
    ...
    netcdf.putAtt(ncid, varid_vart, 'comment', 'OceanSITES quality flagging for Variance Threshold QC test. Threshold set to m2/s2.');
    netcdf.putAtt(ncid, varid_vart, 'FillValue', netcdf.getConstant('NC_FILL_SHORT'));
    netcdf.putAtt(ncid, varid_vart, 'scale_factor', single(1));
    netcdf.putAtt(ncid, varid_vart, 'add_offset', single(0));
    ...
    varid_velt = netcdf.defVar(ncid, 'VELT_QC', 'short', [dimid_range dimid_bearing dimid_depth dimid_t]);
    netcdf.defVarDeflate(ncid, varid_velt, true, true, 6);
    netcdf.putAtt(ncid, varid_velt, 'long_name', 'Velocity Threshold Quality Flags');
    netcdf.putAtt(ncid, varid_velt, 'valid_range', int16( [0 9]));
    netcdf.putAtt(ncid, varid_velt, 'flag_values', int16( [0 1 2 3 4 7 8 9]));
    netcdf.putAtt(ncid, varid_velt, 'flag_meanings', 'unknown good_data probably_good_data potentially_correctable_bad_data bad_data nominal_value interpolated_value missing_value');
    netcdf.putAtt(ncid, varid_velt, 'comment', 'OceanSITES quality flagging for Velocity Threshold QC test. Threshold set to m/s.');
    netcdf.putAtt(ncid, varid_velt, 'FillValue', netcdf.getConstant('NC_FILL_SHORT'));
    netcdf.putAtt(ncid, varid_velt, 'scale_factor', single(1));
    netcdf.putAtt(ncid, varid_velt, 'add_offset', single(0));
    netcdf.putAtt(ncid, varid_velt, 'units', '1');
    netcdf.putAtt(ncid, varid_velt, 'coordinates', 'LONGITUDE LATITUDE');
    
    % Average Radial Bearing QC Flag
    varid_avrb = netcdf.defVar(ncid, 'AVRB_QC', 'short', dimid_qmxl);
    netcdf.defVarDeflate(ncid, varid_avrb, true, true, 6);
    netcdf.putAtt(ncid, varid_avrb, 'long_name', 'Average Radial Bearing Quality Flags');
    ...
    netcdf.putAtt(ncid, varid_avrb, 'comment', 'OceanSITES quality flagging for Average Radial Bearing QC test. Threshold set to deg.');
    netcdf.putAtt(ncid, varid_avrb, 'FillValue', netcdf.getConstant('NC_FILL_SHORT'));
    netcdf.putAtt(ncid, varid_avrb, 'scale_factor', single(1));
    netcdf.putAtt(ncid, varid_avrb, 'add_offset', single(0));
    ...
    % Site latitude
    varid_slat = netcdf.defVar(ncid, 'SLAT', 'float', dimid_nsit);
    netcdf.defVarDeflate(ncid, varid_slat, true, true, 6);
    netcdf.putAtt(ncid, varid_slat, 'long_name', 'Contributing radar site latitudes');
    netcdf.putAtt(ncid, varid_slat, 'valid_range', single( [-180 180] ));
    netcdf.putAtt(ncid, varid_slat, 'FillValue', netcdf.getConstant('NC_FILL_FLOAT'));
    netcdf.putAtt(ncid, varid_slat, 'scale_factor', single(1));
    netcdf.putAtt(ncid, varid_slat, 'add_offset', single(0));
    netcdf.putAtt(ncid, varid_slat, 'units', 'degrees_north');
    
    % Site longitude
    varid_slon = netcdf.defVar(ncid, 'SLON', 'float', dimid_nsit);
    netcdf.defVarDeflate(ncid, varid_slon, true, true, 6);
    netcdf.putAtt(ncid, varid_slon, 'long_name', 'Contributing radar site longitudes');
    netcdf.putAtt(ncid, varid_slon, 'valid_range', single( [-90 90] ));
    netcdf.putAtt(ncid, varid_slon, 'FillValue', netcdf.getConstant('NC_FILL_FLOAT'));
    netcdf.putAtt(ncid, varid_slon, 'scale_factor', single(1));
    netcdf.putAtt(ncid, varid_slon, 'add_offset', single(0));
    netcdf.putAtt(ncid, varid_slon, 'units', 'degrees_east');
    
    % Site codes
    varid_scod = netcdf.defVar(ncid, 'SCOD', 'char', [dimid_smxl dimid_nsit]);
    netcdf.defVarDeflate(ncid, varid_scod, true, true, 6);
    netcdf.putAtt(ncid, varid_scod, 'long_name', 'Contributing radar site codes');
    netcdf.putAtt(ncid, varid_scod, 'comment', 'RDLI: Ideal Antenna Pattern is applied to the radial data. RDLM: Measured Antenna Pattern is applied to the radial data.');
    ...
    netcdf.putVar(ncid, varid_vart, varianceThreshold_QCflag);
    netcdf.putVar(ncid, varid_velt, velocityThreshold_QCflag);
    ...
    netcdf.putVar(ncid, varid_slat, siteLat);
    netcdf.putVar(ncid, varid_slon, siteLon);
    netcdf.putVar(ncid, varid_scod, siteCode);
